...
*/
me.skills = ["Javascript", "Turbo Pascal", "HTML", "CSS"];
/* ESERCIZIO F
  Crea un pezzo di codice per aggiungere un nuovo elemento all'array "skills" contenuto nell'oggetto "me".
 */
me.skills.push("linus");
...
function splitMe(str) {
  return str
    ...
    .map(() => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase());
}
...
console.log(deleteOne("Grande", true));
console.log(deleteOne("Roby", false));
...
console.log(isThisAnEmail("robyal@example.com"));
...
console.log(howManyDays("1975-11-05"));
...
let myObject = { name: "Roberto A", age: 49, city: "New York" };
...
function newestMovie(movies) {
  ...
  return movies.reduce((newest, movie) => {
    return parseInt(movie.Year) > parseInt(newest.Year) ? movie : newest;
  });
}
...
function onlyInLastMillennium(movies) {
  return movies.filter((movie) => parseInt(movie.Year) < 2000); // Filtra i film con anno < 2000
}
  ...
  return movies.reduce(
    (result, movie) => {
      if (movie.Title.toLowerCase().includes(searchString.toLowerCase())) {
        result.match.push(movie); // Aggiunge ai match
      } else {
        result.unmatch.push(movie); // Aggiunge agli unmatch
      }
      return result;
    },
    { match: [], unmatch: [] }
...
*/
function addToList(newItemText) {
  const list = document.getElementById("myList");
  const newItem = document.createElement("li");
  newItem.textContent = newItemText;
  list.appendChild(newItem);
}
addToList("Nuovo Elemento");
/* ESERCIZIO 25
